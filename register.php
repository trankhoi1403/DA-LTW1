...
<?php include "header.php"; ?>
    ...
    <?php if ($row) :?>
        <div class="alert alert-danger" role="alert">
            Vui lòng chọn email khác, lỗi: Trùng email
        </div>
    <?php exit(); endif;?>
    <?php 
        global $db;
        $stmt = $db->prepare("INSERT INTO myuser(email, password, username, fullname, phonenumber) VALUES (?,?,?,?,?)");
        $stmt->execute(array($email, password_hash($password, PASSWORD_DEFAULT), $username, $fullname, $phonenumber));
        //$row = $stmt->fetch(PDO::FETCH_ASSOC);
    ?>
        <div class="alert alert-primary" role="alert">
            Đăng ký thành công. <a href="login.php">CLick vào đây để đăng nhập lại</a>
        </div>
    <?php endif; ?>
<?php
include "footer.php";
